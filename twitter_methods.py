...
def get_by_type(method_type,query, limit= 500):
...
def getTweets(query,numtweets):
  ...
  for i, tweet in enumerate(tw.TwitterSearchScraper(query).get_items()):
      ...
      if gm.data_prep(tweet.rawContent):    #if not null and have more than 3 words append to list 
        tweets_list.append([tweet.user.username, gm.data_prep(tweet.rawContent)])
      else:
